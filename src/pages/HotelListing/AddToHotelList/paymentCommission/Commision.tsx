...
import RequireDeposits from "./components/requireDeposits/RequireDeposits";
import PaymentInformation from "./components/paymentInformation/PaymentInformation";
import Cancellation from "./components/defaultCancellation/Cancellation";
...
const Commision: React.FC = () => {
  ...
  //navigation
  const nextPage = "/photos";
  const previousPage = "/propertyPolicy";

  // local variables
  const [inputBoxValue, setInputBoxValue] = useState("");

  const handleOption1Change = (isChecked: boolean) => {
    setOption1Checked(isChecked);
  };

  const handleOption2Change = (isChecked: boolean) => {
    setOption2Checked(isChecked);
  };

  const handleOption3Change = (isChecked: boolean) => {
    setOption3Checked(isChecked);
  };

  const handleAgreement = (isChecked: boolean) => {
    setAgreeItBestPrice(isChecked);
  };

  const handleCancellationChange = (event: ChangeEvent<HTMLInputElement>) => {
    setSelectedCancellation(event.target.value);
  };

  const handleRefundPercentageChange = (
    event: ChangeEvent<HTMLInputElement>
  ) => {
    setRefundPercentage(event.target.value);
  };

  const handleInputBoxChange = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handlePaymentMethodChange = (option: string) => {
    if (selectedPaymentMethods.includes(option)) {
      setSelectedPaymentMethods(
        selectedPaymentMethods.filter((method) => method !== option)
      );
    } else {
      setSelectedPaymentMethods([...selectedPaymentMethods, option]);
    }
  };
  ...
  return (
    <div>
      <UserContextProvider>
        <Navbar />
        <div className="propteryType-background commision-container">
          <ScrollHeader />
          <h1 className="commission-title">Payment and Commission</h1>
          {isLoading ? (
            <div
              style={{
                display: "flex",
            ...
            <>
              <RequireDeposits
                ...
                onOption3Change={handleOption3Change}
              />
              <PaymentInformation
                paymentInfo={["Credit Card", "PayPal", "Bank Transfer", "Cash"]}
              ...
              <Cancellation
                ...
                handleRefundPercentageChange={handleRefundPercentageChange}
              ...
              <Taxes
                ...
                handleTaxOptionChange={handleTaxOptionChange}
                handleVatPercentageChange={handleVatPercentageChange}
