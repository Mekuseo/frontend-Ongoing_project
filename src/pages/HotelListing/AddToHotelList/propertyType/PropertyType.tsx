...
import CustomRoomType from "./components/roomType/RoomType";
import RoomCapacity from "./components/roomCapacity/RoomCapacity";
...
import RoomTypeNumber from "./components/roomTypeNumber/roomTypeNumber";
import BedType from "./components/bedType/BedType";
import { Checkbox } from "@mui/material";
import RoomTypeImageUploader from "../../../../component/stay/addtohotel/roomTypeImageUploader";
// import RoomTypeImageUploader from "../../../../component/stay/addtohotel/roomTypeImageUploader";
...
const PropertyType = () => {
  ...
  const [nightDealsDiscount, setNightDealsDiscount] = useState("");
  ...
  //navigation
  const nextPage = "/propertyPolicy";
  const previousPage = "/amenities";
  // to be reworked on

  // const handleNumBedroomsChange = (num: React.SetStateAction<number>) => {
  //   setNumBedrooms(num);
  // };
            ...
            {isLoading ? (
              ...
              <>
                ...
                <CustomRoomType
                  ...
                  setRoomType={setNumber_Of_Room_Type}
                />

                {/* {roomTypeName.map((name, index) => (
                  <h1 key={index}>
                    Room {index + 1}: {name}
                  </h1>
                ))} */}
                ...
                {Array.from({ length: number_of_room_type }).map((_, index) => (
                  <div className="propertyType-repeat" key={index}>
                    <hr className="propertyType-spacer" />
                    <h2
                      style={{
                        fontFamily: "volkhov, sans-serif",
                      }}
                      className="proptyperoomtypenumber"
                    >
                      Room type {index + 1}
                    </h2>
                    <RoomTypeNumber
                      roomTypeName={roomTypeName[index]}
                      setRoomTypeName={(value) => {
                        const updatedRoomTypeNames = [...roomTypeName];
                        updatedRoomTypeNames[index] = value;
                        setRoomTypeName(updatedRoomTypeNames);
                      }}
                    />

                    <RoomCapacity
                      numOfBedrooms={numOfBedrooms[index]}
                      setOfNumBedrooms={(value) => {
                        const updatedRoomTypeNun = [...numOfBedrooms];
                        updatedRoomTypeNun[index] = value;
                        setOfNumBedrooms(updatedRoomTypeNun);
                      }}
                    />
                    {/* <h1>{numOfBedrooms[index]}</h1> */}
                    <RoomTypeDrop
                      selectedCategory={selectedCategory[index]}
                      onSelectCategory={(value) => {
                        const updatedRoomTypeDrop = [...selectedCategory];
                        updatedRoomTypeDrop[index] = value;
                        onSelectCategory(updatedRoomTypeDrop);
                      }}
                    />

                    <RoomTypeImageUploader
                      uploadedImages={images[index] || []}
                    ...
                    <p>{selectedCategory[index]}</p>
                    <BedType
                      checkboxComponent={<Checkbox color="success" />}
                      selectedRoomTypeAmenities={
                        selectedRoomTypeAmenities[index]
                      }
                      setSelectedRoomTypeAmenities={(value) => {
                        setSelectedRoomTypeAmenities(
                    ...
                    <RoomClassDrop
                      ...
                      selectedCategoryRoom={selectedCategoryRoom[index]}
                      setSelectedCategoryRoom={(value) => {
                        const updatedRoomCategory = [...selectedCategoryRoom];
                        updatedRoomCategory[index] = value;
                        setSelectedCategoryRoom(updatedRoomCategory);
                      }}
                    />
                    <RoomRates
                      ...
                      nightlyRate={
                        nightlyRate[index] !== null ? nightlyRate[index] : 0
                      }
                      onNightlyRateChange={(value) => {
                        const updateNightly = [...nightlyRate];
                        updateNightly[index] = value;
                        setNightlyRate(updateNightly);
                      }}
                      weeklyDiscount={
                        weeklyDiscount[index] !== null
                          ? weeklyDiscount[index]
                          : 0
                      }
                      onWeeklyRateChange={(value) => {
                        const updateWeeklyDiscount = [...weeklyDiscount];
                        updateWeeklyDiscount[index] = value;
                        setWeeklyDiscount(updateWeeklyDiscount);
                      }}
                      monthlyDiscount={
                        monthlyDiscount[index] !== null
                          ? monthlyDiscount[index]
                          : 0
                      }
                      onMonthlyRateChange={(value) => {
                        const updateMonthlyDiscount = [...monthlyDiscount];
                        updateMonthlyDiscount[index] = value;
                        setMonthlyDiscount(updateMonthlyDiscount);
                      }}
                      roomDiscount={
                        roomDiscount[index] !== null ? roomDiscount[index] : 0
                      }
                      onRoomDiscountChange={(value) => {
                        const updateRoomDiscount = [...roomDiscount];
                        updateRoomDiscount[index] = value;
                        setRoomDiscount(updateRoomDiscount);
                      }}
                      late_Night_Deal_Off={
                        late_Night_Deal_Off[index] !== null
                          ? late_Night_Deal_Off[index]
                          : 0
                      }
                      setLate_Night_Deal_Off={(value) => {
                        const updateLate_Night_Deal_Off = [
                          ...late_Night_Deal_Off,
                        ];
                        updateLate_Night_Deal_Off[index] = value;
                        setLate_Night_Deal_Off(updateLate_Night_Deal_Off);
                      }}
                      automate_late_night_deals={
                        automate_late_night_deals[index]
                      }
                      setAutomate_Late_Night_Deals={(value) => {
                        const updateAutomate_late_night_deals = [
                          ...automate_late_night_deals,
                        ];
                        updateAutomate_late_night_deals[index] = value;
                        setAutomate_Late_Night_Deals(
                          updateAutomate_late_night_deals
                        );
                      }}
                      selectedDates={selectedDates[index]}
                      ...
                      hasSelectedDates={hasSelectedDates[index]}
                      setHasSelectedDates={(value) => {
                        const updateHasSelectedDates = [...hasSelectedDates];
                        updateHasSelectedDates[index] = value;
                        setHasSelectedDates(updateHasSelectedDates);
                      }}
